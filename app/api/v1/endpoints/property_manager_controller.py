...
def create_property_manager(property_manager_create: PropertyManagerCreate, db: Session = Depends(get_db)):
    ...
    existing_property_manager = db.query(PropertyManager).filter(PropertyManager.email == property_manager.email).first()
...
def update_property_manager(property_manager_id: int, property_manager_update: PropertyManagerUpdate, db: Session = Depends(get_db)):
